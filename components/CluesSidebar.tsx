...
import type { Clue } from "@/lib/mysteryData";
	...
	return (
		<Card className="h-full flex flex-col border-none shadow-none bg-transparent">
			<CardHeader className="px-0">
				...
				<CardDescription>Level {currentLevel} Investigation</CardDescription>
			</CardHeader>
				...
				{lockedClues.length > 0 && (
					<div className="space-y-4">
						...
						{lockedClues.map((clue) => (
							<div
								key={clue.id}
								className="p-4 bg-muted/20 border border-dashed border-border/50 rounded-xl opacity-60"
							>
								<div className="flex items-center gap-2 text-muted-foreground">
									<Lock className="w-4 h-4" />
									<span className="text-xs font-medium">
										Unlock after {clue.unlockLevel} queries
									</span>
								</div>
							</div>
						))}
...
function ClueItem({ clue }: { clue: Clue }) {
	...
	return (
		<div className="p-4 bg-white border border-border/50 rounded-xl space-y-3">
			...
			{revealed ? (
				<p className="text-foreground text-sm leading-relaxed font-medium animate-in fade-in duration-500">
					{clue.hint}
				</p>
			) : (
				<button
			...
			<div className="pt-2 border-t border-border/50">
				...
				<div className="flex flex-wrap gap-1.5">
					{clue.queryKeywords.map((kw) => (
						<code
							key={kw}
							className="px-1.5 py-0.5 bg-background border border-border rounded text-[11px] font-mono text-primary"
						>
							{kw}
						</code>
					))}
